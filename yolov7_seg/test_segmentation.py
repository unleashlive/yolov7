    ...
    def postprocess(self, detections, proto, image):
        ...
        for i, det in enumerate(detections):  # per image
            ...
            if len(det):
                ...
                contours, _ = cv2.findContours(cv2.resize(my_mask, (image.shape[1], image.shape[0])), cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)
                new_image = cv2.fillPoly(img=new_image, pts=contours, color=(0,255,0))
                ...
                *xyxy, conf, cls = reversed(det[:, :6])[0]
                ...
                cv2.namedWindow('box', cv2.WINDOW_NORMAL | cv2.WINDOW_KEEPRATIO)  # allow window resize (Linux)
                cv2.resizeWindow('box', im0.shape[1], im0.shape[0])
                cv2.imshow('box', im0)
                cv2.waitKey() 
